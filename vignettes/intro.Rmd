...
In this example we have two datasets, messy_caterpillars contains information about the average weight of caterpillars, and clean_caterpillars is the number of caterpillars you found last summer. You want to calculate the average total weight by species of the caterpillars you saw, but there are misspellings and typos that make merging these two datasets a pain.
...
You might also notice that there is weight data for more species than you saw this summer, and with cat_replace() we can match the mispelled names to the correct names, and exclude the caterpillars that you did not find. 
...
cat_match(messy_caterpillars$`Caterpillar Species`,clean_caterpillars$species,return_dists = T) %>% arrange(desc(dists))
```
Now we know to set a threshold of ~.12 when using cat_replace()
```{r using outputs inner_join}
messy_caterpillars$`Caterpillar Species` = cat_replace(messy_caterpillars$`Caterpillar Species`,clean_caterpillars$species,threshold=0.12)

left_join(clean_caterpillars,messy_caterpillars)
```
